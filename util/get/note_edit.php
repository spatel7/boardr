<?php
	...
	function getNote($a) {
		...
		echo "<div><div><font color='black' style='font-size: 17px;'><textarea id='notecontent' style='background-color: #ffffd9; width: 374px; border: 0px solid #f0f0f0; padding: 0px; font-family: times new roman; font-size: 17px;' onchange='javscript:(notecontent.style.height = notecontent.scrollHeight + \"px\");' onkeyup='javscript:(notecontent.style.height = notecontent.scrollHeight + \"px\");' onkeydown='javscript:(notecontent.style.height = notecontent.scrollHeight + \"px\");' onfocus='javscript:(notecontent.style.height = notecontent.scrollHeight + \"px\");'>".$r['scribble']."</textarea></font></div><div style='margin-top: 15px; width: 100%; float: left;'><div style='float: left; max-width: 90%;'><font color='gray'>".getTime($time1,$r['time'])." - ".getBoardName($r['bid'])."</font></div><div style='float: right;'><a id='muters' href='javascript:closeOverlay();'><font color='#410041'><b>Close this box</b></font></a></div></div></div>";																					
	}
